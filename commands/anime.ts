...
import Discord, { ActionRowBuilder, ButtonBuilder, EmbedBuilder, ButtonStyle, ApplicationCommandOptionType } from "discord.js";
  ...
  callback: async ({ member, interaction, message, text, client }) => {
    ...
    async function embedBuilder(result: any, embed: any, row: any) {
      ...
      if (result.alternative_titles) {
        ...
        embed.addFields({name: `Synonyms`, value: `\`\`\`${synonyms}\`\`\``, inline: true});
      }
      ...
      embed.addFields({name: 'Rating', value: `\`\`\`\n${rating}\`\`\``, inline: true});
      ...
      embed.addFields({name: "Media Info", value: `\`\`\`${media}\`\`\``, inline: true});
      ...
      embed.addFields({name: 'Studios', value: `\`\`\`${studios}\`\`\``, inline: true});
