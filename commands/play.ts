        ...
        if (searchTerm.startsWith('http')) {
          if (searchTerm.includes('spotify')) {
            ...
            if (searchTerm.includes('track')) {
              ...
              if (song.status == "SUCCESS" && song.results[0] && song.results[0]?.name.includes(spotifyTrack?.name) && song.results[0]?.artist.includes(spotifyTrack?.artists[0]?.name)) {
                await buildSong(song);
              }
              else {
                embed.setTitle('❌ No streams were found for the provided song');
                return embed;
              }
            }
          }
