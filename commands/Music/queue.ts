...
export default {
  ...
  callback: async ({ interaction }: { interaction: Interaction }) => {
    ...
    try {
      if (!int.member.voice.channelId && int.options?.getSubcommand() == 'clear') {
      ...
      if (oldConnection && oldConnection.joinConfig.channelId != int.member.voice?.channelId && int.options?.getSubcommand() == 'clear') {
        return await int.reply({
          embeds: [
            new EmbedBuilder()
      ...
      if (!queue.tracks[1]) {
        return await int.reply({
          embeds: [
            new EmbedBuilder()
              .setColor(11553764)
              .setDescription("**⚠️ There are no tracks currently added to the queue**")
          ],
          ephemeral: true
        }).catch(() => null);
      }
